...
export function Yarimil6KSQBSQ({ route }: Props) {
  ...
  const [ksq6, setKsq6] = useState<string>('');
  ...
  const ksq6Ref = React.useRef<TextInput>(null);
    ...
    <SafeAreaView style={{ flex: 1 }}>
      <View style={calcStyles.container}>
        ...
        <TextInput
          ...
          onBlur={() => setFocus(null)}
          returnKeyType="next"
          onSubmitEditing={() => ksq6Ref.current?.focus()}
        />
        <TextInput
          ref={ksq6Ref}
          ...
          keyboardType="numeric"
          value={ksq6}
          onChangeText={(text) => handleKsqInput(text, setKsq6)}
...
const calcStyles = StyleSheet.create({
  ...
  input: {
    ...
    marginRight: 5, // Aralarındaki boşluk 1px
